    ...
    try {
      if (initialTrustedPosts?.length === 0) {
        ...
        if (posts) {
          ...
          dispatch(
            getResponderInitialPosts({ initialResponderPosts: posts.documents })
          );
        }
      } else {
  ...
  return (
    ...
    >
      ...
      <div id="Home_RIGHT_LEFT" className={`flex gap-5 px-8 py-5 w-full`}>
        ...
        <div ref={homeLeft} className="Home_Left flex flex-col gap-6">
          {posts?.map((post) => {
            return (
              <div
                className={`RespondersSectionPage_PostCard ${
                  isDarkModeOn ? "darkMode" : ""
                }`}
                key={post?.$id}
          ...
          {isLoading && hasMorePostInTrustedPost && (
