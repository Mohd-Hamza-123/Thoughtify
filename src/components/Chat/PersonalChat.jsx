...
const PersonalChat = ({ receiverDetails, ChatRoomID }) => {
  ...
  const deleteAllMessages = async () => {
    messages.forEach((msg) => {
      deleteDoc(doc(db, "messages", msg?.$id)).then((res) => {
        setNotificationPopMsgNature((prev) => true);
        ...
        <div className="room--container">
          <div id="PersonalChat_Receiver_div" className="flex justify-between">
            ...
            <div className="PersonalChat_Ellipsis_Vertical">
              {messages?.length !== 0 && (
                <i
                  onClick={() => setisDeleteAllMsgActive((prev) => !prev)}
              ...
              {messages?.length !== 0 && (
                <p
                  ...
                  className={`${isDeleteAllMsgActive ? "active" : ""}`}
                >
                  Clear Chat
                </p>
              )}
          ...
          <div
            ref={messagesDiv}
            id="PersonalChat_Messages"
            onClick={() => setisDeleteAllMsgActive((prev) => false)}
          >
              ...
              return (
                <div key={message.$id} className={"message--wrapper"}>
                  <div className="message--header">
                    <button
                      className={`PersonalChatDeleteBtn ${
                        message.userId === userData.$id ? "" : "hidden"
                      }`}
                  ...
                  <div
                    className={`message--body ${
                      message?.userId === userData.$id
                        ? "PersonalChatMyColor self-end"
                        : "bg-black self-start PersonalChatYourColor"
                    }`}
                  >
